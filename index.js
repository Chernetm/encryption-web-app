...
function decryptRSA(encryptedText) {
    const decrypted = crypto.privateDecrypt(
        ...
        Buffer.from(encryptedText, "base64") // Decode from base64 before decryption
    );
...
app.post("/encrypt", (req, res) => {
    ...
    try {
        if (algorithm === "rsa") {
            ...
            return res.status(400).json({ error: "Invalid algorithm. Use 'rsa', 'otp', '3des', or 'aes'." });
        }
        res.json({ encrypted: encryptedText });
    } catch (error) {
        res.status(500).json({ error: error.message });
...
app.post("/decrypt", (req, res) => {
    ...
    if (!encryptedText || !algorithm) {
        return res.status(400).json({ error: "Missing encryptedText or algorithm" });
    }

    let decryptedText;
    try {
        if (algorithm === "rsa") {
            decryptedText = decryptRSA(encryptedText);
        } else if (algorithm === "otp") {
            decryptedText = decryptOTP(encryptedText, key);
        } else if (algorithm === "3des") {
            decryptedText = decrypt3DES(encryptedText, key);
